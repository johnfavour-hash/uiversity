...
import { Search, Plus, MoreHorizontal, X, ChevronLeft, Eye, Edit2, Trash2, ArrowLeft } from 'lucide-react';
...
const initialAccountsData: AccountRecord[] = [
  { code: 'UNI-001', role: 'Department Admin', name: 'Computer Science', email: 'admin@tin.edu.ng', phone: '2348012345678', status: 'Active' },
  { code: 'UNI-002', role: 'Faculty Admin', name: 'Sciences', email: 'admin@tin.edu.ng', phone: '2348012345678', status: 'Not Active' },
  { code: 'UNI-003', role: 'Department Admin', name: 'Animal and Env...', email: 'grace@gyu.edu.ng', phone: '2348012345678', status: 'Active' },
  { code: 'UNI-004', role: 'Department Admin', name: 'Crop and Soil Sc...', email: 'htyyy@tin.edu.ng', phone: '2348012345678', status: 'Not Active' },
  { code: 'UNI-005', role: 'Faculty Admin', name: 'Engineering', email: 'noes@edu.ng', phone: '2348012345678', status: 'Active' },
  { code: 'UNI-006', role: 'Department Admin', name: 'Computer Science', email: 'shavel@tin.edu.ng', phone: '2348012345678', status: 'Pending' },
  { code: 'UNI-007', role: 'Department Admin', name: 'Computer Science', email: 'leviloe@edu.ng', phone: '2348012345678', status: 'Not Certified' },
  { code: 'UNI-008', role: 'Faculty Admin', name: 'Computer Science', email: 'jaysha@tin.edu.ng', phone: '2348012345678', status: 'Certified' },
  { code: 'UNI-009', role: 'Faculty Admin', name: 'Computer Science', email: 'fredalo@.edu.ng', phone: '2348012345678', status: 'Not Certified' },
  { code: 'UNI-010', role: 'Faculty Admin', name: 'Computer Science', email: 'shavel@tin.edu.ng', phone: 'shavel@tin.edu.ng', status: 'Certified' },
  { code: 'UNI-011', role: 'Faculty Admin', name: 'Computer Science', email: 'hope@tin.edu.ng', phone: 'hope@tin.edu.ng', status: 'Certified' },
  { code: 'UNI-012', role: 'Faculty Admin', name: 'Computer Science', email: 'saint@tin.edu.ng', phone: 'saint@tin.edu.ng', status: 'Certified' },
];
...
const AdminProfileView = ({ admin, onBack }: { admin: AccountRecord; onBack: () => void }) => {
    ...
    <div className="space-y-6 animate-in fade-in slide-in-from-bottom-4 duration-500 pb-10">
      ...
      <div className="bg-white p-8 md:p-12 rounded-[2rem] border border-slate-100 shadow-sm flex flex-col md:flex-row justify-between items-center gap-8">
        <div className="text-center md:text-left">
          <h2 className="text-3xl md:text-4xl font-bold text-slate-800 mb-2">{admin.name}</h2>
      ...
      <div className="bg-white p-8 md:p-12 rounded-[2rem] border border-slate-100 shadow-sm space-y-12">
        <section className="space-y-8">
          <h3 className="text-xl font-bold text-slate-900">Admin Information</h3>
          <div className="grid grid-cols-1 md:grid-cols-2 gap-x-12 gap-y-8">
            <div className="space-y-3">
              <label className="text-sm font-bold text-slate-800 block">Name</label>
              <input 
                type="text" 
            ...
            <div className="space-y-3">
              <label className="text-sm font-bold text-slate-800 block">Admin Role</label>
              <input 
                type="text" 
            ...
            <div className="space-y-3">
              <label className="text-sm font-bold text-slate-800 block">Official Email</label>
              <input 
                type="email" 
        ...
        <section className="space-y-8">
          <h3 className="text-xl font-bold text-slate-900">Roles & Permission</h3>
          <div className="grid grid-cols-1 md:grid-cols-2 gap-x-12">
            {permissions.map((isChecked, i) => (
              <div key={i} className="flex items-center justify-between py-5 border-b border-slate-100 last:border-0 md:[&:nth-child(7)]:border-0 md:[&:nth-child(8)]:border-0">
                <span className="text-sm font-medium text-slate-700">Create Programs</span>
                <ToggleSwitch checked={isChecked} onChange={() => togglePermission(i)} />
              </div>
            ))}
          </div>
        </section>
        ...
        <div className="flex flex-col sm:flex-row justify-end items-center gap-4 pt-6">
          ...
          <button 
            className="w-full sm:w-auto px-10 py-3 bg-[#22c55e] hover:bg-emerald-600 text-white font-bold rounded-lg shadow-sm transition-all text-sm"
          >
            Save Changes
          </button>
        </div>
              ...
              <form onSubmit={handleSubmit} className="space-y-5 md:space-y-6 mt-2 md:mt-4">
                <div className="space-y-1 md:space-y-2">
                  <label className="text-xs md:text-sm font-bold text-slate-800">
                    {adminType === 'Department Admin' ? 'Name of Department' : 'Name of Faculty'} <span className="text-orange-600">*</span>
                  </label>
                  <input
                    type="text"
                    required
                ...
                <div className="space-y-1 md:space-y-2">
                  ...
                  <input
                    ...
                    onChange={(e) => setFormData({ ...formData, parentFaculty: e.target.value })}
                  />
                </div>
          ...
          <table className="w-full text-left border-collapse min-w-[900px]">
            <thead>
              <tr className="border-b border-slate-100 bg-slate-50/50">
                ...
                <th className="p-4 md:p-6 text-slate-400 font-bold text-[10px] md:text-xs uppercase tracking-widest whitespace-nowrap">Code</th>
                <th className="p-4 md:p-6 text-slate-400 font-bold text-[10px] md:text-xs uppercase tracking-widest whitespace-nowrap">Admin Role</th>
                ...
                <th className="p-4 md:p-6 text-slate-400 font-bold text-[10px] md:text-xs uppercase tracking-widest whitespace-nowrap">Email Address</th>
                ...
                <th className="p-4 md:p-6 text-slate-400 font-bold text-[10px] md:text-xs uppercase tracking-widest text-center whitespace-nowrap">Status</th>
            ...
            <tbody className="divide-y divide-slate-50">
              {filteredAccounts.map((row, idx) => (
                <tr key={idx} className="hover:bg-slate-50/80 transition-all duration-200 group relative">
                  ...
                  <td className="p-4 md:p-6 text-[10px] md:text-xs font-black text-slate-800 tracking-tight whitespace-nowrap">{row.code}</td>
                  <td className="p-4 md:p-6 text-[10px] md:text-xs font-bold text-slate-600 whitespace-nowrap">{row.role}</td>
                  <td className="p-4 md:p-6 text-[10px] md:text-xs font-bold text-slate-900 whitespace-nowrap">{row.name}</td>
